...
void Database::addClassPeriod(ClassPeriod* classPeriod, int id_type, Module *mod) {
    ...
    list<Group*>::iterator it = classPeriod->GetGroupList()->begin();
    list<Group*>::const_iterator itMax = classPeriod->GetGroupList()->end();
    for(; it!=itMax; it++) {
        cout << "insert into assign values('"<<classPeriod->GetId()<<"', '"<<(*it)->GetId()<<"')"<<endl;
        history << "insert into assign values('"<<classPeriod->GetId()<<"', '"<<(*it)->GetId()<<"')"<<endl;
    }
}
